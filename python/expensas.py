...
# ============ MANEJO DE ARCHIVOS ============
def cargar_archivo_html(nombre):
    """Carga archivo HTML desde carpeta de plantillas"""
    ruta = f"{Config.PLANTILLAS_DIR}/{nombre}.html"
    
    if not os.path.exists(ruta):
        print(f"❌ ERROR: No se encontró {nombre}.html en {Config.PLANTILLAS_DIR}")
        return None
...
def log_envio(tipo, destinatario, depto, exito, error=None):
    ...
    estado = "✓ ENVIADO" if exito else f"✗ ERROR: {error}"
...
def mostrar_resumen_previo_expensas(df, mes_expensas):
    ...
    for _, row in df.head(limite).iterrows():
        ...
        pdf1_ok = "✓" if os.path.exists(f"{Config.PDFS_DIR}/Expensas {n}.pdf") else "✗"
        pdf2_ok = "✓" if os.path.exists(f"{Config.PDFS_DIR}/Detalle expensas {n}.pdf") else "✗"
    ...
    print("="*60)
    print(f"\n📧 ASUNTO: COMUNICACIONES - Liquidación de Expensas {mes_expensas} - DEPTO [X]")
    print("\n📝 CUERPO (extracto):\n")
...
def enviar_expensas_mensuales():
    """Envía expensas mensuales a todas las unidades"""
    imprimir_header("📧 ENVÍO DE EXPENSAS MENSUALES", Config.MODO_TEST)
    ...
    if faltantes:
        print(f"\n⚠️  FALTAN {len(faltantes)} PDFs (primeros 5):")
        for pdf in faltantes[:5]:
            print(f"   - {pdf}")
        if len(faltantes) > 5:
            print(f"   ... y {len(faltantes)-5} más")
        print("\n⚠️  Se saltearán las unidades con PDFs faltantes")
    else:
        print("✓ Todos los PDFs encontrados\n")
    
    # Cargar plantillas
    plantilla = cargar_archivo_html('expensas')
    firma_html = cargar_archivo_html('firma')

    if not plantilla or not firma_html:
        print("\n❌ No se pueden enviar emails sin las plantillas HTML")
        return
    
    # Preparar variables
    mes_expensas = obtener_mes_expensas()
    print(f"📅 Período: {mes_expensas}\n")
    
    # Mostrar resumen
    mostrar_resumen_previo_expensas(df, mes_expensas)
    plantilla_sample = plantilla.replace('{mes_expensas}', mes_expensas) + firma_html
    print(html_a_texto_simple(plantilla_sample))
    
    print("\n" + "="*60)
    print(f"📊 TOTAL A ENVIAR: {len(df)} emails")
    if Config.MODO_TEST:
        print(f"🧪 MODO TEST: Todos se enviarán a {Config.EMAIL_TEST}")
    print("="*60 + "\n")
    
    if not confirmar_accion("¿CONFIRMAR ENVÍO?"):
...
def enviar_avisos_corte_luz():
    """Envía avisos de corte de luz a morosos"""
    imprimir_header("⚡ ENVÍO DE AVISOS DE CORTE DE LUZ")
    
    print("📋 Cargando planilla de recaudación...")
    nombres_morosos = obtener_morosos()
    print(f"⚠️  {len(nombres_morosos)} morosos detectados\n")
    
    if len(nombres_morosos) == 0:
        print("✓ No hay morosos. ¡Todos pagaron!")
        return
    
    lista_envio, no_encontrados = preparar_lista_envio_morosos(nombres_morosos)
    
    if no_encontrados:
        print(f"⚠️  No se encontró email para:")
        for n in no_encontrados:
            print(f"   - {n}")
        print()
    
    if len(lista_envio) == 0:
        print("❌ No se pudo obtener ningún email")
        return
    ...
    for item in lista_envio:
        cuerpo_html = plantilla.replace('{fecha_corte}', fecha_corte) + firma_html
        ...
        if exito:
            ...
            if Config.MODO_TEST:
                print(f"  ✓ {item['depto']} - [{item['email_prop']}] → {destinatario}{cc_text}")
            else:
                print(f"  ✓ {item['depto']} ({destinatario}){cc_text}")
            enviados += 1
            log_envio('corte_luz', item['email_prop'], item['depto'], True)
        else:
            print(f"  ✗ {item['depto']} ({item['email_prop']}) - Error: {resultado}")
            errores += 1
...
# ============ FUNCIÓN 3: AVISOS GENERALES ============
def enviar_avisos_generales(asunto=None):
    """Envía avisos generales personalizables"""
    imprimir_header("📢 ENVÍO DE AVISOS GENERALES")
    ...
    for _, row in df.iterrows():
        cuerpo_html = (plantilla.replace('{nombre}', row['Nombre'])
                                .replace('{depto}', row['Depto']) + firma_html)
        ...
        if exito:
            cc_text = f" (CC: {cc_real})" if cc_real else ""
            if Config.MODO_TEST:
                print(f"  ✓ {row['Depto']} - [{row['Email_Propietario']}] → {destinatario}{cc_text}")
            else:
                print(f"  ✓ {row['Depto']} ({destinatario}){cc_text}")
            enviados += 1
            log_envio('aviso_general', row['Email_Propietario'], row['Depto'], True)
        else:
            print(f"  ✗ {row['Depto']} ({row['Email_Propietario']}) - Error: {resultado}")
            errores += 1
...
def menu_principal():
    ...
    while True:
        print("\n" + "="*60)
        print("🏢 SISTEMA DE ENVÍO DE EXPENSAS")
        if Config.MODO_TEST:
            print(f"🧪 [MODO TEST ACTIVADO - Email: {Config.EMAIL_TEST}]")
        print("="*60)
